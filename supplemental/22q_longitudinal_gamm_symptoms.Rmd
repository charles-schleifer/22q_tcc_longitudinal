...
fpn_iq_td <- lmerTest::lmer("IQ_full ~ Frontoparietal.combat + SEX + (1|SUBJECTID)", data=sx_td, REML=TRUE, na.action = "na.omit")
fpn_iq_td_summary <- summary(fpn_iq_td)
fpn_iq_td_summary
...
# log sips p sum
SIPS_log_som <- ggplot(data=sx_del, aes(x=log_sips_p_sum, y=Somatomotor.combat))+
  geom_point()+
  geom_smooth(method = "lm")+
  #ggtitle("Somatomotor")+
  ylab("TCC")+
  xlab("log(SIPS_positive + 1)")+
  theme_classic() 
SIPS_log_fpn <- ggplot(data=sx_del, aes(x=log_sips_p_sum, y=Frontoparietal.combat))+
  geom_point()+
  geom_smooth(method = "lm")+
  #ggtitle("Frontoparietal")+
  ylab("")+
...
#SIPS_log <- ggarrange(SIPS_log_som, SIPS_log_fpn,SIPS_log_cin, nrow=1, ncol = 3)
SIPS_del <- SIPS_del_som+SIPS_del_fpn + plot_annotation(title="22qDel Positive Symptoms")
SIPS_log <- SIPS_log_som+SIPS_log_fpn + plot_annotation(title="log Transformed Positive Symptoms")
SIPS_plot <- ggarrange(SIPS_del, SIPS_log, nrow=2, ncol=1)
#SIPS_plot <- SIPS_del/SIPS_log + plot_layout(tag_level = 'new') +  plot_annotation(tag_levels = list(c('22qDel', 'Control')))
SIPS_plot
#SIPS_del
...
## frontoparietal 
print("FPN SIPS log(p) 22qDel")
fpn_sips_logp_del <- lmerTest::lmer("Frontoparietal.combat ~ log_sips_p_sum + SEX + (1|SUBJECTID)", data=sx_td, REML=TRUE, na.action = "na.omit")
fpn_sips_logp_del_summary <- summary(fpn_sips_logp_del)
fpn_sips_logp_del_summary
## somatomotor 
print("SOM SIPS log(p) 22qDel")
som_sips_logp_del <- lmerTest::lmer("Somatomotor.combat ~ log_sips_p_sum + SEX + (1|SUBJECTID)", data=sx_td, REML=TRUE, na.action = "na.omit")
som_sips_logp_del_summary <- summary(som_sips_logp_del)
som_sips_logp_del_summary
