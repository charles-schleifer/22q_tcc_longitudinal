...
```{r message=FALSE, warning=FALSE, paged.print=FALSE, results='hide'}
# read roi key 
roi_key <- read.csv(file.path(project,"supplemental/anatomical_roi_key.csv"), header = TRUE)
...
read_tc_results_anat <- function(sdir, fname, sesh, site){
  print(sesh)
...
tc_name_trio <- "resting_network_TCC_matrix_Atlas_s_hpss_res-mVWMWB1d_lpss_FS_anatomical.csv"
tc_name_prisma <- "restingAP_network_TCC_matrix_Atlas_s_hpss_res-mVWMWB1d_lpss_FS_anatomical.csv"

# read for trio and prisma then combine
trio_tc <- lapply(trio_sessions, function(s) read_tc_results_anat(sesh=s,site="trio",sdir=trio_dir,fname=tc_name_trio)) %>% do.call(rbind,.) %>% as.data.frame
prisma_tc <- lapply(prisma_sessions, function(s) read_tc_results_anat(sesh=s,site="prisma",sdir=prisma_dir,fname=tc_name_prisma)) %>% do.call(rbind,.) %>% as.data.frame
all_tc <- rbind(trio_tc,prisma_tc) 

# subset to within network values
wnet_tc <- filter(all_tc, Thalamus == Cortex)

# use regex to get columns with hemisphere and network name
wnet_tc$hemi <- wnet_tc$Cortex %>% gsub("_[^_]+$", "", .)
wnet_tc$network <- wnet_tc$Cortex %>% gsub("^.*\\_","", .)

# average left and right hemispheres
bilat_tc <- NULL
for (s in unique(wnet_tc$MRI_S_ID)){
  for (n in unique(wnet_tc$network)){
    # get L and R mean for a given region in a given subject
    lr <- filter(wnet_tc, MRI_S_ID==s & network == n)
    mean <- mean(lr$pearson_r_Fz)
    st <- lr$site %>% unique
    out <- data.frame(MRI_S_ID=s, site=st, NETWORK=n, TC_Fz=mean)
    # rbind to previous data
    bilat_tc <- rbind(bilat_tc, out)
  }
}

# cast to wide for combat
setDT(bilat_tc)
all_tc_wide <- reshape2::dcast(bilat_tc, MRI_S_ID + site ~ NETWORK, value.var="TC_Fz") 
```
...
plot_gamm_resid_tc <- function(name,xlab="", ylab="",ylim=c(-1,1),xlim=c(5.9,23)){
  ...
  ggplot()+
    ...
    ylab(ylab)+
    xlab(xlab)+
    ggtitle(str_to_title(paste(gsub("_"," ",gsub(".combat","",name)),"Thalamocortical Connectivity")))
}
...
plot_age_dervs <- function(derv,ylab="", xlab="",legend_position="right",ageranges="",maxderv=maxderv_all){
  # get derivative of specified smooth from gam
  #derv <- derivatives(object=gam, term=smooth,n_sim=100000)
...
gam_derv_arrange <- ggarrange(gam_derv_plot("motor.combat"),gam_derv_plot("somatosensory.combat"),gam_derv_plot("parietal.combat"),gam_derv_plot("prefrontal.combat"),gam_derv_plot("temporal.combat"),gam_derv_plot("visual.combat"), nrow=2, ncol=3)
gam_derv_arrange

gam_legend <- get_legend(plot_gamm_resid_tc(name="motor.combat", ylab="TCC"))
derv_legend <- get_legend(plot_age_dervs(dervs_del[["motor.combat"]], ylab="22qDel",ageranges=sig_ages_del[["motor.combat"]], maxderv=maxderv_all))
gam_derv_legend <- ggarrange(gam_legend, derv_legend, ncol=1)
gam_derv_legend

ggsave(plot=gam_derv_arrange, filename=file.path(project,"figures/anatomical/anatomical_tcc_gsr.pdf"), width=16, height=11.5, units="in", device="pdf")
#ggsave(plot=gam_derv_arrange, filename=file.path(project,"figures/anatomical/anatomical_tcc_gsr.png"), width=16, height=12, units="in", device="png", dpi=300, bg = "white")

ggsave(plot=gam_derv_legend, filename=file.path(project,"figures/anatomical/anatomical_tcc_gsr_legend.pdf"), units="in", device="pdf")
#ggsave(plot=gam_derv_legend, filename=file.path(project,"figures/anatomical/anatomical_tcc_gsr_legend.png"), units="in", device="png", dpi=300, bg = "white")
```
...
colnames(sig_ages_del_df) <- "sig_change"
...
colnames(sig_ages_hcs_df) <- "sig_change"
...
out_age_pvals_derv <- arrange(out_age_pvals_derv, Group, factor(Network,levels=c("motor","somatosensory","parietal","prefrontal","temporal","visual")))
out_age_pvals_derv
# write output
...
colnames(sig_dif_ages_df) <- "sig_difference"
sig_dif_ages_df$Network <- gsub(".combat","",rownames(sig_dif_ages_df))
sig_dif_ages_df <- sig_dif_ages_df[,c("Network","sig_difference")]
#sig_dif_ages_df
gamm_table_final <- merge(x=out_age_pvals_derv, y=sig_dif_ages_df, by="Network",all.x=TRUE)
gamm_table_final <- arrange(gamm_table_final, Group, factor(Network,levels=c("motor","somatosensory","parietal","prefrontal","temporal","visual")))

# write output
write.csv(gamm_table_final, file=file.path(project,"/figures/anatomical/anatomical_gamm.csv"), row.names = FALSE, quote=FALSE)
# to maintain rounding and trailing zeros, open blank excel workbook, import text file, and import CSV changing column types to text, then save as xlsx
```
