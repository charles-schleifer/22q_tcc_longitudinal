...
demo_mri_tc_hcs_del <- merge(x=demo_mri_tc_hcs_del, y=summpsych[,c("SUBJECTID","CONVERTEDVISITNUM","PSYTYPE")], by=c("SUBJECTID","CONVERTEDVISITNUM"), all.x=T) %>% rename("psych_meds" = "PSYTYPE")
demo_mri_tc_hcs_del$apd <- demo_mri_tc_hcs_del$psych_meds == "antipsychotic"
demo_mri_tc_hcs_del[which(is.na(demo_mri_tc_hcs_del$apd)),"apd"] <- FALSE
demo_mri_tc_hcs_del$apd %<>% as.numeric
...
demovars <- c("MRI_S_ID","SUBJECTID","site","SUBJECT_IDENTITY","AGE","SEX","visit_index","apd")
features <- tc_names
idvar <- 'MRI_S_ID'
batchvar <- 'site'
timevar <- 'visit_index'
formula <- 'SUBJECT_IDENTITY + AGE + SEX + apd'
ranef <- '(1|SUBJECTID)'
...
gamm_tc_all_combat <- lapply(tc_names_combat,function(name) gam(reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","apd","SEX","site","s(SUBJECTID, bs=\"re\")"), response=name),selection=TRUE,method="REML",data=demo_mri_tc_hcs_del_combat))
...
# write output
write.csv(out_age_pvals_derv, file=file.path(project,"/figures/sensitivity_analyses/22q_tcc_gamm_age_results_gsr_antipsychotic.csv"), row.names = FALSE, quote=FALSE)
