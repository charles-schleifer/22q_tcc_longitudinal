...
demo_mri_tc_hcs_del_combat_gam <- demo_mri_tc_hcs_del_combat 
demo_mri_tc_hcs_del_combat_gam$SUBJECTID %<>% as.factor
demo_mri_tc_hcs_del_combat_gam$site <- demo_mri_tc_hcs_del_combat_gam$site %>% gsub("trio","0",.) %>% gsub("prisma","1",.) %>% as.numeric 
demo_mri_tc_hcs_del_combat_gam$SEX <- demo_mri_tc_hcs_del_combat_gam$SEX %>% gsub("F","0",.) %>% gsub("M","1",.) %>% as.numeric 


# generalized additive mixed model
gamm_tc_all_combat <- lapply(tc_names_combat,function(name) gam(reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","site","s(SUBJECTID, bs=\"re\")"), response=name),selection=TRUE,method="REML",data=demo_mri_tc_hcs_del_combat_gam))
...
maria's code looks at partial effects, here we'll check summed effects: https://cran.r-project.org/web/packages/itsadug/vignettes/inspect.html
https://jacolienvanrij.com/Tutorials/GAMM.html
using predict_gam
```{r}
demo_combat_na_del <- filter(demo_combat_na_gam, SUBJECT_IDENTITY=="PATIENT-DEL") %>% droplevels
demo_combat_na_hcs <- filter(demo_combat_na_gam, SUBJECT_IDENTITY=="CONTROL") %>% droplevels

pred_del <- predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", values = list(SUBJECT_IDENTITY="PATIENT-DEL", SUBJECTID=NULL, eTIVscaled = mean(demo_combat_na_del$eTIVscaled), site=mean(demo_combat_na_del$site), SEX=mean(demo_combat_na_del$SEX)), length_out=100)

pred_dup <- predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", values = list(SUBJECT_IDENTITY="PATIENT-DUP", SUBJECTID=NULL, eTIVscaled = mean(demo_combat_na_dup$eTIVscaled), site=mean(demo_combat_na_dup$site), SEX=mean(demo_combat_na_dup$SEX)), length_out=100)

pred_hcs <- predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", values = list(SUBJECT_IDENTITY="CONTROL", SUBJECTID=NULL, eTIVscaled = mean(demo_combat_na_hcs$eTIVscaled), site=mean(demo_combat_na_hcs$site), SEX=mean(demo_combat_na_hcs$SEX)), length_out=100)

pred <- rbind(pred_del,pred_dup,pred_hcs)
pred$sehi <- pred$fit + pred$se.fit
pred$selo <- pred$fit - pred$se.fit


#pred <- predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", values = list(SUBJECTID=NULL, eTIVscaled = mean(demo_combat_na_gam$eTIVscaled), site=mean(demo_combat_na_gam$site), SEX=mean(demo_combat_na_gam$SEX)))

#pred <- predict_gam(gamm_combat[[1]], exclude="s(SUBJECTID)", values = list(SUBJECTID=NULL, eTIVscaled = mean(demo_combat_na_gam$eTIVscaled), site=mean(demo_combat_na_gam$site), SEX=mean(demo_combat_na_gam$SEX)))

#pred <- predict_gam(gamm_combat[[1]], values = list(eTIVscaled = mean(demo_combat_na_gam$eTIVscaled), site=mean(demo_combat_na_gam$site), SEX=mean(demo_combat_na_gam$SEX)))


ggplot()+
  #geom_smooth_ci(SUBJECT_IDENTITY)+
  #geom_smooth(aes(group=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY, ymax=sehi, ymin=selo), method=NULL)+
  geom_ribbon(data=pred,aes(x=AGE,group=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY,fill=SUBJECT_IDENTITY,ymin=selo, ymax=sehi),alpha=0.5)+
  geom_line(data=pred,aes(x=AGE, y=fit,group=SUBJECT_IDENTITY, lty=SUBJECT_IDENTITY),color="black")+
  #geom_line(data=pred,aes(x=AGE, y=fit,group=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY))+
  ...
  geom_point(data=demo_combat_na, aes_string(x="AGE", y=sc_names_combat[[1]], color="SUBJECT_IDENTITY", fill="SUBJECT_IDENTITY",shape="site"),color="black") +
  scale_shape_manual(values = c(24, 21)) 
```
using plot_smooth
```{r}
plot_smooth(gamm_tc_all_combat[[5]],view="AGE", plot_all = "SUBJECT_IDENTITY", rm.ranef = TRUE, se=1)
plot_smooth(gamm_tc_all_combat[[7]],view="AGE", plot_all = "SUBJECT_IDENTITY", rm.ranef = TRUE, se=1)
