...
#ggsave(plot=waterfall, filename=file.path(project,"figures/demographics/waterfall_age.svg"), width=4, height=6, device = "svg")
#ggsave(plot=waterfall, filename=file.path(project,"figures/demographics/waterfall_age.png"), width=4, height=6, device = "png")
...
plot_gamm_points_tc_combat <- function(name,xlab="", ylab="",ylim=c(-1,2),xlim=c(5.9,23)){
...
maxderv <- lapply(dervs_del[c("Somatomotor.combat","Frontoparietal.combat")], function(d) d[,"derivative"]) %>% do.call(rbind,.) %>% abs %>% max %>% round(digits=3)
...
ggsave(plot=plot_gamms_dervs_fpn_legend, filename=file.path(project,"figures/sensitivity_analyses/fpn_gamm_raw_noGSR.pdf"),device="pdf",bg="transparent", width=10, height=5.5, dpi=1000, units="in")
...
ggsave(plot=plot_gamms_dervs_som_legend, filename=file.path(project,"figures/sensitivity_analyses/som_gamm_raw_noGSR.pdf"),device="pdf",bg="transparent", width=10, height=5.5, dpi=1000, units="in")
```
...
# write output
#write.csv(out_age_pvals_derv, file=file.path(project,"/figures/sensitivity_analyses/22q_tcc_gamm_age_results_noGSR.csv"), row.names = FALSE, quote=FALSE)
