...
---
## Overview
winsorized version of analysis
...
GAMM for harmonized winsorized TC
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
gamm_tc_all_combat <- lapply(tc_names_winsor,function(name) gam(reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","site","s(SUBJECTID, bs=\"re\")"), response=name),selection=TRUE,method="REML",data=demo_mri_tc_hcs_del_combat))
names(gamm_tc_all_combat) <- tc_names_winsor

# smooth estimate
smooth_tc_all_combat <- lapply(gamm_tc_all_combat, function(g) smooth_estimates_se_b0(gamm=g, smooth="s(AGE)", n=(40*12)))
names(smooth_tc_all_combat) <- tc_names_winsor
...
out_age_pvals$Network <- gsub(".combat.winsor","",out_age_pvals$Network)
...
sig_ages_del_df$Network <- gsub(".combat.winsor","",rownames(sig_ages_del_df))
...
sig_ages_hcs_df$Network <- gsub(".combat.winsor","",rownames(sig_ages_hcs_df))
...
# write output
write.csv(out_age_pvals_derv, file=file.path(project,"/figures/sensitivity_analyses/22q_tcc_gamm_age_results_gsr_wins.csv"), row.names = FALSE, quote=FALSE)
