...
---
## Overview
This script uses Generalized Additive Mixed Models (GAMMs) to model non-parametric age trajectories for cortical network global connectivity in 22qDel patients and typical controls. A random effects implementation of the ComBat algorithm (longCombat) is used to harmonize data from multiple sites.  
...
## Load individual GBC CSVs
computed by 22q_multisite_networkGBC_save_individual.R and saved as a CSV with one value per network representing the z-transformed pearson correlation between the signal in the cortical networks and the whole brain 
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
tc_name_trio <- "resting_fc_network_Cortex_GBC_Atlas_s_hpss_res-mVWMWB1d_lpss_CABNP.csv"
tc_name_prisma <- "restingAP_fc_network_Cortex_GBC_Atlas_s_hpss_res-mVWMWB1d_lpss_CABNP.csv"
...
all_tc <- rbind(trio_tc,prisma_tc) %>% filter(!is.na(GBC_Fz))
...
setDT(all_tc)
all_tc_wide <- reshape2::dcast(all_tc, MRI_S_ID + site ~ NETWORK, value.var="GBC_Fz") 
```
...
plot all GBC smooths
```{r}
# function to plot gamm and scatter
plot_gamm_points_tc_combat <- function(name,xlab="", ylab="",ylim=c(-1,1),xlim=c(5.9,22)){
  ...
  ylab(ylab)+
  xlab(xlab)+
  ggtitle(paste(name,"Network GBC GAMM"))+
