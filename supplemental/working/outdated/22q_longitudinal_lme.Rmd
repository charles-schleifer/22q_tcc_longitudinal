...
linear mixed effects for age*group interaction
```{r warning=FALSE}
# combat feature names
tc_names_combat <- paste(tc_names,".combat",sep="")

# linear random intercept model age interaction
lme_tc_all_combat <- lapply(tc_names_combat,function(name) summary(lmerTest::lmer(reformulate(c("AGE*SUBJECT_IDENTITY","SEX","site","(1|SUBJECTID)"), response=name),REML=TRUE,data=demo_mri_tc_hcs_del_combat)))
names(lme_tc_all_combat) <- tc_names_combat

group_age_interactions <- lapply(lme_tc_all_combat, function(s)s$coefficients["AGE:SUBJECT_IDENTITYPATIENT-DEL",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% data.frame 
colnames(group_age_interactions) <- c("beta","p_value")
group_age_interactions$fdr_q <- p.adjust(group_age_interactions$p_value , method="fdr")

# linear random intercept model group difference
lme_gdif_tc_all_combat <- lapply(tc_names_combat,function(name) summary(lmerTest::lmer(reformulate(c("SUBJECT_IDENTITY","AGE","SEX","site","(1|SUBJECTID)"), response=name),REML=TRUE,data=demo_mri_tc_hcs_del_combat)))
names(lme_gdif_tc_all_combat) <- tc_names_combat

gdif_lme <- lapply(lme_gdif_tc_all_combat, function(s)s$coefficients["SUBJECT_IDENTITYPATIENT-DEL",c("Estimate","Pr(>|t|)")]) %>% do.call(rbind,.) %>% data.frame 
colnames(gdif_lme) <- c("beta","p_value")
gdif_lme$fdr_q <- p.adjust(gdif_lme$p_value , method="fdr")
```
