...
---
## Overview
Replicate 22q_longitudinal_gamm.Rmd using input data calculated from fMRI without global signal regression
...
## Load individual TC connectivity CSVs ***no GSR***
...
load whole thalamus connectivity
```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
# file name to look for ***No GSR***
wtc_name_trio <- "resting_fc_network_WholeThal_Cortex_Atlas_s_hpss_res-mVWM1d_lpss_CABNP.csv"
wtc_name_prisma <- "restingAP_fc_network_WholeThal_Cortex_Atlas_s_hpss_res-mVWM1d_lpss_CABNP.csv"

# read for trio and prisma then combine
trio_wtc <- lapply(trio_sessions, function(s) read_tc_results(sesh=s,site="trio",sdir=trio_dir,fname=wtc_name_trio)) %>% do.call(rbind,.) %>% as.data.frame
prisma_wtc <- lapply(prisma_sessions, function(s) read_tc_results(sesh=s,site="prisma",sdir=prisma_dir,fname=wtc_name_prisma)) %>% do.call(rbind,.) %>% as.data.frame
all_wtc <- rbind(trio_wtc,prisma_wtc) %>% filter(!is.na(whole_thal_TC_Fz))

# replaces dashes with underscores for network names
all_wtc$NETWORK <- paste0("wt_",all_wtc$NETWORK) %>% gsub("-","_",.)

# cast to wide for combat
setDT(all_wtc)
all_wtc_wide <- reshape2::dcast(all_wtc, MRI_S_ID + site ~ NETWORK, value.var="whole_thal_TC_Fz") 
```
...
merge and harmonize whole-thalamus
```{r}
demo_mri_wtc <- merge(x=ucla_demo_hcs_del, y=all_wtc_wide, by="MRI_S_ID")
demo_mri_wtc_hcs_del <- demo_mri_wtc %>% filter(SUBJECT_IDENTITY=="CONTROL" | SUBJECT_IDENTITY =="PATIENT-DEL")

# change class to factor
demo_mri_wtc_hcs_del$SUBJECT_IDENTITY <- factor(demo_mri_wtc_hcs_del$SUBJECT_IDENTITY)
demo_mri_wtc_hcs_del$SUBJECTID <- factor(demo_mri_wtc_hcs_del$SUBJECTID)
demo_mri_wtc_hcs_del$site <- factor(demo_mri_wtc_hcs_del$site)
demo_mri_wtc_hcs_del$MRI_S_ID <- factor(demo_mri_wtc_hcs_del$MRI_S_ID)

# list of TC network names
wtc_names <- names(all_wtc_wide)[which(!(names(all_wtc_wide) %in% c("MRI_S_ID", "site")))]
...
# input df should not have any unused columns or package will error
# one row per unique scan
combat_input_wtc_hcs_del <- demo_mri_wtc_hcs_del[,c(demovars,features)]

# run longCombat
wtc_net_combat <- longCombat(data=combat_input_wtc_hcs_del, idvar=idvar, timevar=timevar, batchvar=batchvar, features=features, formula=formula, ranef=ranef)

# get the harmonized data
wtc_net_combat_data <- wtc_net_combat$data_combat

# merge combat back with original
demo_mri_wtc_hcs_del_combat <- merge(x=demo_mri_wtc_hcs_del, y=wtc_net_combat_data, by=c("MRI_S_ID","visit_index","site"))
```
...
plot_gamm_points_tc_combat <- function(name,xlab="", ylab="",ylim=c(-1,1.5),xlim=c(5.9,22)){
  ...
  ylab(ylab)+
  xlab(xlab)+
  ggtitle(paste(name,"Thalamocortical No GSR GAMM"))+
...
plot_gamm_points_wtc_combat <- function(name,xlab="", ylab="",ylim=c(-1,1.1),xlim=c(5.9,22)){
  ...
  ylab(ylab)+
  xlab(xlab)+
  ggtitle(paste(name,"Whole-thalamus No GSR GAMM"))+
  scale_fill_manual(values = c("CONTROL" = "deepskyblue", "PATIENT-DEL" = "red")) +
  scale_color_manual(values = c("CONTROL" = "deepskyblue", "PATIENT-DEL" = "red")) +
