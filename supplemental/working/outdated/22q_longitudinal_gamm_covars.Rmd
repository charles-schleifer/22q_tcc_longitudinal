...
demovars <- c("MRI_S_ID","SUBJECTID","site","SUBJECT_IDENTITY","AGE","SEX","visit_index","percent_udvarsme")
features <- tc_names
idvar <- 'MRI_S_ID'
batchvar <- 'site'
timevar <- 'visit_index'
...
combat_input_tc_hcs_del <- demo_mri_tc_hcs_del_m[,c(demovars,features)]
...
demo_mri_tc_hcs_del_combat <- merge(x=demo_mri_tc_hcs_del_m, y=tc_net_combat_data, by=c("MRI_S_ID","visit_index","site"))
```
...
gamm_tc_all_combat <- lapply(tc_names_combat,function(name) gam(reformulate(c("s(AGE,by=SUBJECT_IDENTITY)","SUBJECT_IDENTITY","SEX","site","percent_udvarsme","s(SUBJECTID, bs=\"re\")"), response=name),selection=TRUE,method="REML",data=demo_mri_tc_hcs_del_combat))
...
# for export, skip to derivitave section to merge with significant age ranges
...
out_age_pvals_derv <- arrange(out_age_pvals_derv, Group, factor(Network,levels=c("Frontoparietal","Somatomotor","Cingulo_Operc99ular","Auditory","Default","Dorsal_Attention","Posterior_Multimodal","Visual1","Visual2")))
out_age_pvals_derv
out_age_pvals_derv$Network <- gsub("_"," ",out_age_pvals_derv$Network)
# write output
write.csv(out_age_pvals_derv, file=file.path(project,"/figures/sensitivity_analyses/22q_tcc_gamm_age_results_gsr_movement.csv"), row.names = FALSE, quote=FALSE)
...
df_demo_table_bl <- merge(x=df_demo_table_bl, y=edin_result_fixed[c("SUBJECTID","CONVERTEDVISITNUM","hand")], by=c("SUBJECTID","CONVERTEDVISITNUM"), all.x=T)
