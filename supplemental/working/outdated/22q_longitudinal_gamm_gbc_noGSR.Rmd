...
## Load individual GBC CSVs ***No GSR***
...
tc_name_trio <- "resting_fc_network_Cortex_GBC_Atlas_s_hpss_res-mVWM1d_lpss_CABNP.csv"
tc_name_prisma <- "restingAP_fc_network_Cortex_GBC_Atlas_s_hpss_res-mVWM1d_lpss_CABNP.csv"
...
plot_gamm_points_tc_combat <- function(name,xlab="", ylab="",ylim=c(-0.5,2),xlim=c(5.9,22)){
  ...
  ylab(ylab)+
  xlab(xlab)+
  ggtitle(paste(name,"Network GBC GAMM, No GSR"))+
...
Test mean global signal group and age effects
```{r message=FALSE, warning=FALSE}

# function to read thalamocortical results and add columns for roi pair name, site, and ID 
read_wbts <- function(sdir, fname, sesh){
  print(sesh)
  input <- read.csv(file.path(sdir,sesh,"images/functional",fname))
  return(input)
}
...
# file name to look for ***No GSR***
wb_name_trio <- "resting_movement_scrubbed_nuisance_regressors.csv"
wb_name_prisma <- "restingAP_movement_scrubbed_nuisance_regressors.csv"

# read for trio and prisma 
trio_wb <- lapply(trio_sessions, function(s) read_wbts(sesh=s,sdir=trio_dir,fname=wb_name_trio)) 
prisma_wb <- lapply(prisma_sessions, function(s) read_wbts(sesh=s,sdir=prisma_dir,fname=wb_name_prisma)) 

# get mean WB signal for each sesh
trio_wb_mean <- lapply(trio_wb, function(r) mean(as.numeric(r$WB))) %>% do.call(rbind,.)
prisma_wb_mean <- lapply(prisma_wb, function(r) mean(as.numeric(r$WB))) %>% do.call(rbind,.)

trio_wb_means_df <- data.frame(MRI_S_ID=trio_sessions, WB_mean=trio_wb_mean)
prisma_wb_means_df <- data.frame(MRI_S_ID=prisma_sessions, WB_mean=prisma_wb_mean)
all_wb_means <- rbind(trio_wb_means_df,prisma_wb_means_df) 

# get std dev of WB signal for each sesh
trio_wb_sd <- lapply(trio_wb, function(r) sd(as.numeric(r$WB))) %>% do.call(rbind,.)
prisma_wb_sd <- lapply(prisma_wb, function(r) sd(as.numeric(r$WB))) %>% do.call(rbind,.)

trio_wb_sd_df <- data.frame(MRI_S_ID=trio_sessions, WB_sd=trio_wb_sd)
prisma_wb_sd_df <- data.frame(MRI_S_ID=prisma_sessions, WB_sd=prisma_wb_sd)
all_wb_sd <- rbind(trio_wb_sd_df,prisma_wb_sd_df) 

# merge with demo
demo_mri_wb <- merge(x=demo_mri_tc_hcs_del_combat, y=all_wb_means, by="MRI_S_ID")
demo_mri_wb <- merge(x=demo_mri_wb, y=all_wb_sd, by="MRI_S_ID")

# test linear mixed effects model of WB signal
wb_lme <- lmerTest::lmer(formula="WB_mean ~ SUBJECT_IDENTITY + AGE + site + SEX + (1|SUBJECTID)", data=demo_mri_wb, REML=TRUE)
wb_lme_sum <- summary(wb_lme)
wb_lme_sum

# test linear mixed effects model of WB sd
wb_lme_sd <- lmerTest::lmer(formula="WB_sd ~ SUBJECT_IDENTITY + AGE + site + SEX + (1|SUBJECTID)", data=demo_mri_wb, REML=TRUE)
wb_lme_sd_sum <- summary(wb_lme_sd)
wb_lme_sd_sum

# plot WB signal by group
ggplot(data=demo_mri_wb, aes(WB_mean, fill=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY, y=..count..))+
  geom_density(kernel="gaussian", alpha=0.5)+
  scale_fill_manual(values=c("lightblue","red"))+
  scale_color_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("Mean whole-brain BOLD signal distributions")

ggplot(data=filter(demo_mri_wb, site =="trio" ), aes(WB_mean, fill=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY, y=..count..))+
  geom_density(kernel="gaussian", alpha=0.5)+
  scale_fill_manual(values=c("lightblue","red"))+
  scale_color_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("Mean whole-brain BOLD signal distributions, trio only")


ggplot(data=filter(demo_mri_wb, site =="prisma" ), aes(WB_mean, fill=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY, y=..count..))+
  geom_density(kernel="gaussian", alpha=0.5)+
  scale_fill_manual(values=c("lightblue","red"))+
  scale_color_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("Mean whole-brain BOLD signal distributions, prisma only")


# plot wb signal vs age
ggplot(data=demo_mri_wb, aes(y=WB_mean,x=AGE, fill=SUBJECT_IDENTITY, color=SUBJECT_IDENTITY))+
  geom_point()+
  geom_smooth(method="lm")+
  scale_fill_manual(values=c("lightblue","red"))+
  scale_color_manual(values=c("lightblue","red"))+
  theme_classic()+
  ggtitle("mean global signal vs age")
