...
plot_gamms_fpn <- plot_gamm_points_tc_combat(name="Frontoparietal.combat", ylab="TCC")
...
plot_gamms_som <- plot_gamm_points_tc_combat(name="Somatomotor.combat", ylab="TCC")
...
gam_derv_plot <- function(name){
  plot_gamm <- plot_gamm_points_tc_combat(name=name, ylab="TCC")
...
gam_legend <- get_legend(plot_gamm_points_tc_combat(name="Frontoparietal.combat", ylab="TCC"))
...
df_demo_table_all <- demo_mri_tc_hcs_del_combat
...
get_sips <- function(r,demo,sips){
   ...
   if(nrow(sips_sesh) < 1){
     ...
     df_out[,c("SIPS_sleep","SIPS_p_sum","SIPS_n_sum","SIPS_d_sum","SIPS_g_sum","SIPS_total","SIPS_psychosis_6","SIPS_psspectrum_3")] <- rep(NA,times=8)
   }else if(nrow(sips_sesh) > 1){
     # if more than one match for sub+visit, note error
     df_out[,c("SIPS_sleep","SIPS_p_sum","SIPS_n_sum","SIPS_d_sum","SIPS_g_sum","SIPS_total", "SIPS_psychosis_6","SIPS_psspectrum_3")] <- rep("ERROR-duplicates",times=8)
   }else{
     # get SIPS sleep measure
     df_out[,"SIPS_sleep"] <- sips_sesh[c("G1SEV")]
     # get SIPS P scores
...
df_demo_table_all <- merge(x=df_demo_table_all, y=demo_table_sips[,c("SUBJECTID","CONVERTEDVISITNUM","SIPS_sleep","SIPS_total","SIPS_psspectrum_3","SIPS_p_sum")], by=c("SUBJECTID","CONVERTEDVISITNUM"), all.x=T) %>% rename("SIPS_prodromal" = "SIPS_psspectrum_3")
...
aasp <- df_all_ucla$aasp[,c("SUBJECTID","CONVERTEDVISITNUM","TOTQUAD1","TOTQUAD2","TOTQUAD3","TOTQUAD4")]
df_demo_table_all <- merge(x=df_demo_table_all, y=aasp, by=c("SUBJECTID","CONVERTEDVISITNUM"), all.x=TRUE)

# short sensory profile
ssp <- df_all_ucla$SSP[,c("SUBJECTID","CONVERTEDVISITNUM", "TACTILE","TASTE","MOVEMENT","UNDER","AUDITORY","WEAK","VISUAL","SHORSENSTOTAL")]
df_demo_table_all <- merge(x=df_demo_table_all, y=ssp, by=c("SUBJECTID","CONVERTEDVISITNUM"), all.x=TRUE)



```

linear mixed effects models of TCC on symptoms
```{r}
df_del <- filter(df_demo_table_all, SUBJECT_IDENTITY=="PATIENT-DEL")

# Prodromal (t/f)
lme_som_pro <- lmerTest::lmer(formula="SIPS_prodromal ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_pro, ddf="Kenward-Roger")

lme_fpn_pro <- lmerTest::lmer(formula="SIPS_prodromal ~ Frontoparietal.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_pro, ddf="Kenward-Roger")

# SIPS positive
lme_som_sipsp <- lmerTest::lmer(formula="SIPS_p_sum ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sipsp, ddf="Kenward-Roger")

lme_fpn_sipsp <- lmerTest::lmer(formula="SIPS_p_sum ~ Frontoparietal.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_sipsp, ddf="Kenward-Roger")

# IQ
lme_som_iq <- lmerTest::lmer(formula="IQ_full ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_iq, ddf="Kenward-Roger")

lme_fpn_iq <- lmerTest::lmer(formula="IQ_full ~ Frontoparietal.combat  + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_iq, ddf="Kenward-Roger")

# SIPS sleep
lme_som_sleep <- lmerTest::lmer(formula="SIPS_sleep ~ Somatomotor.combat+ SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sleep, ddf="Kenward-Roger")

lme_fpn_sleep <- lmerTest::lmer(formula="SIPS_sleep ~ Frontoparietal.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_sleep, ddf="Kenward-Roger")

# ASD
lme_som_asd <- lmerTest::lmer(formula="summPsych_ASD ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_asd, ddf="Kenward-Roger")

lme_fpn_asd <- lmerTest::lmer(formula="summPsych_ASD ~ Frontoparietal.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_asd, ddf="Kenward-Roger")

# som vs sx plots
som_sx_scatter <- function(df, var){
  ggplot(data=df, aes_string(x="Somatomotor.combat", y=var))+
    geom_point()+
    ggtitle(var)
}

lapply(c("SIPS_p_sum","SIPS_sleep","IQ_full","summPsych_ASD","SIPS_prodromal"), function(v) som_sx_scatter(df=df_del, var=v))

# AASP
filter(df_demo_table_all, !is.na(TOTQUAD1))$AGE %>% summary

lme_som_aasp1 <- lmerTest::lmer(formula="TOTQUAD1 ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_aasp1, ddf="Kenward-Roger")

lme_som_aasp2 <- lmerTest::lmer(formula="TOTQUAD2 ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_aasp2, ddf="Kenward-Roger")

lme_som_aasp3 <- lmerTest::lmer(formula="TOTQUAD3 ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_aasp3, ddf="Kenward-Roger")

lme_som_aasp4 <- lmerTest::lmer(formula="TOTQUAD4 ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_aasp4, ddf="Kenward-Roger")

# SSP
lme_som_sspTot <- lmerTest::lmer(formula="SHORSENSTOTAL ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspTot, ddf="Kenward-Roger")

lme_som_sspTact <- lmerTest::lmer(formula="TACTILE ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspTact, ddf="Kenward-Roger")

lme_som_sspMovement <- lmerTest::lmer(formula="MOVEMENT ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspMovement, ddf="Kenward-Roger")

lme_som_sspAud <- lmerTest::lmer(formula="AUDITORY ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspAud, ddf="Kenward-Roger")

lme_som_sspVis <- lmerTest::lmer(formula="VISUAL ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspVis, ddf="Kenward-Roger")

lme_som_sspUnder <- lmerTest::lmer(formula="UNDER ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspUnder, ddf="Kenward-Roger")

lme_som_sspWeak <- lmerTest::lmer(formula="WEAK ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspWeak, ddf="Kenward-Roger")

lme_som_sspTaste <- lmerTest::lmer(formula="TASTE ~ Somatomotor.combat + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sspTaste, ddf="Kenward-Roger")
...
```{r}
# Prodromal (t/f)
lme_som_pro <- lmerTest::lmer(formula="SIPS_prodromal ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_pro, ddf="Kenward-Roger")

lme_fpn_pro <- lmerTest::lmer(formula="SIPS_prodromal ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_pro, ddf="Kenward-Roger")

# SIPS positive
lme_som_sipsp <- lmerTest::lmer(formula="SIPS_p_sum ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sipsp, ddf="Kenward-Roger")

lme_fpn_sipsp <- lmerTest::lmer(formula="SIPS_p_sum ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_sipsp, ddf="Kenward-Roger")

# IQ
lme_som_iq <- lmerTest::lmer(formula="IQ_full ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_iq, ddf="Kenward-Roger")

lme_fpn_iq <- lmerTest::lmer(formula="IQ_full ~ AGE  + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_iq, ddf="Kenward-Roger")

# SIPS sleep
lme_som_sleep <- lmerTest::lmer(formula="SIPS_sleep ~ AGE+ SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_sleep, ddf="Kenward-Roger")

lme_fpn_sleep <- lmerTest::lmer(formula="SIPS_sleep ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_sleep, ddf="Kenward-Roger")

# ASD
lme_som_asd <- lmerTest::lmer(formula="summPsych_ASD ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_som_asd, ddf="Kenward-Roger")

lme_fpn_asd <- lmerTest::lmer(formula="summPsych_ASD ~ AGE + SEX + (1|SUBJECTID)", data=df_del, REML=TRUE, na.action="na.omit")
summary(lme_fpn_asd, ddf="Kenward-Roger")

# sx vs age plots
age_scatter <- function(df, var){
  ggplot(data=df, aes_string(x="AGE", y=var))+
    geom_point()+
    ggtitle(var)
}

lapply(c("SIPS_p_sum","SIPS_sleep","IQ_full","summPsych_ASD","SIPS_prodromal"), function(v) age_scatter(df=df_del, var=v))
```
