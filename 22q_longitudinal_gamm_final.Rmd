...
waterfall <- ggplot(ucla_demo_waterfall, aes(x=(AGEMONTH/12), y=as.factor(age1), color=Group, fill=Group)) +
  ...
  scale_fill_manual(values = c("CONTROL" = "deepskyblue", "PATIENT-DEL" = "red"), labels=c("Control","22qDel")) +
  scale_color_manual(values = c("CONTROL" = "deepskyblue", "PATIENT-DEL" = "red"), labels=c("Control","22qDel")) +
  guides(fill  = guide_legend(order = 1, override.aes = list(shape=21, size=2,alpha=0.9, fill=c("deepskyblue","red"))),
...
df_demo_table_bl <- filter(demo_mri_tc_hcs_del_combat, visit_index == 1)[,c("SUBJECTID", "CONVERTEDVISITNUM", "MRI_S_ID", "SUBJECT_IDENTITY", "AGE", "SEX", "EDUDAD", "EDUMOM", "EDUYEARS","site")]
...
vars_use <- c("AGE","SEX","hand", "percent_BOLD_scrubbed", "IQ_full", "SIPS_total","SIPS_p_sum","SIPS_prodromal", dx_use, "summPsych_ASD", "psych_meds","visit_counts","avg_interval","site")
...
"site (%) trio"="Siemens Trio scanner, n (%)")
...
```{r}
all_motion <- merge(demo_mri_tc_hcs_del_combat, percent_udvarsme_all, by.x="MRI_S_ID", by.y="sesh")
demo_match_longitudinal <- CreateTableOne(data=all_motion,vars=c("site","percent_udvarsme", "visit_index"),strata="SUBJECT_IDENTITY",addOverall=F,includeNA=T)
demo_match_longitudinal

export_all_motion <- print(demo_match_longitudinal, quote=F, noSpaces=T, printToggle=T, showAllLevels=T)
#write.csv(export_all_motion, file=file.path(project,"figures/demographics/scanner_motion_visit.csv"))
...
```{r}
# SOM group by site interaction 
#som_group_site <- lmerTest::lmer("Somatomotor.combat ~ SUBJECT_IDENTITY * site + (1|SUBJECTID)", data=demo_mri_tc_hcs_del_combat, REML=TRUE, na.action = "na.omit")
#summary(som_group_site)

# SOM group by site by visit interaction 
som_group_site_visit <- lmerTest::lmer("Somatomotor.combat ~ SUBJECT_IDENTITY * site * as.numeric(visit_index) + (1|SUBJECTID)", data=demo_mri_tc_hcs_del_combat, REML=TRUE, na.action = "na.omit")
summary(som_group_site_visit)

# FPN group by site interaction 
#fpn_group_site <- lmerTest::lmer("Frontoparietal.combat ~ SUBJECT_IDENTITY * site + (1|SUBJECTID)", data=demo_mri_tc_hcs_del_combat, REML=TRUE, na.action = "na.omit")
#summary(fpn_group_site)

# FPN group by site by visit interaction 
fpn_group_site_visit <- lmerTest::lmer("Frontoparietal.combat ~ SUBJECT_IDENTITY * site * as.numeric(visit_index) + (1|SUBJECTID)", data=demo_mri_tc_hcs_del_combat, REML=TRUE, na.action = "na.omit")
summary(fpn_group_site_visit)
```


```{r}

# function to do chi square and return p-value for SUBJECT_IDENTITY and site
group_site_chisq <- function(df){
    contingency_table <- table(df$SUBJECT_IDENTITY, df$site)
    chi_square_result <- chisq.test(contingency_table)
    p <- chi_square_result$p.value 
    return(p)
}

visits <- demo_mri_tc_hcs_del_combat$visit_index %>% unique %>% sort

group_site_chisq_visits <- lapply(visits, function(v) group_site_chisq(df=filter(demo_mri_tc_hcs_del_combat, visit_index==v)))
group_site_chisq_visits

# function to get percent trio
percent_trio <- function(vector){
  l <- length(vector)
  ntrio <- sum(vector == "trio")
  percent <- ntrio/l
  return(percent)
}

del_percent_trio_visits <- lapply(visits, function(v) percent_trio(filter(demo_mri_tc_hcs_del_combat, SUBJECT_IDENTITY=="PATIENT-DEL" & visit_index==v)$site))
del_percent_trio_visits


td_percent_trio_visits <- lapply(visits, function(v) percent_trio(filter(demo_mri_tc_hcs_del_combat, SUBJECT_IDENTITY=="CONTROL" & visit_index==v)$site))
td_percent_trio_visits
...
# sink()
# 
# save.image(file=file.path(project,"demographics/data.Rdata"),compress = "bzip2")
# load("~/Dropbox/github/22q_tcc_longitudinal/demographics/data.Rdata")
```

requested during proofing
```{r}
# get estimated degrees of freedom for f-values in table 2
del_f <- lapply(gamm_tc_all_combat, function(g) round(summary(g)$s.table["s(AGE):SUBJECT_IDENTITYPATIENT-DEL","edf"], digits=2))
td_f <- lapply(gamm_tc_all_combat, function(g) round(summary(g)$s.table["s(AGE):SUBJECT_IDENTITYCONTROL","edf"], digits=2))
```
